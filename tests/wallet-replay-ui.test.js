...
test('optional replay fetch failure leaves research usable and cannot render hostile HTML',async()=>{
 ...
 await settle();assert.deepEqual(calls,['/api/hypothesis-replay','/api/wallet-replay','/api/research','/api/wallet-history/a','/api/behavior']);assert.equal(nodes['wallet-count'].textContent,'1');assert.match(nodes.status.textContent,/Snapshot loaded/);
